...
def filter_merge_sort_json(file1, file2, output_file):
    ...
    try:
        with open(file2, "r", encoding="utf-8") as f:
            data = json.load(f)
            if isinstance(data, list):
                filtered_data = [item for item in data if item.get("initial_correct") is True]
...
def process_label_out(input_file1, input_file2, output_file, error_file, error_file_original, pred_error_ids_path):
    error_ids = set()
    correct_data = []
    error_data = []
    ...
    print(f"Saved data corresponding to incorrect IDs from {input_file2} to {error_file_original}")
